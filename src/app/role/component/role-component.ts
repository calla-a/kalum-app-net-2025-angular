...
export class RoleComponent implements OnInit {
  ...
  editRole(id: string, name: string) {
    const dialogRefFormRole = this.dialogForm.open(FormRole, { width: '450px', data: { roleId: id, roleName: name } })
      .afterClosed().subscribe(handlerResult => {
        if (handlerResult == 1) {
          ...
          Swal.fire('Roles', 'Ocurrio un error al actualizar el registro', 'error').then((result) => {
            if (result.isConfirmed) {
              console.log('Error');
            }
          });
  ...
  deleteRole(id: string) {
    Swal.fire({
      ...
      text: "El cambioo no podrá ser revertido",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        this.roleService.deleteRole(id).subscribe(handlerResult => {
          Swal.fire({
            title: "Eliminado!",
            text: `Usted ha eliminado el registro con id ${id}`,
            icon: "success"
          }).then(result => {
            this.getRoles();
          });
  ...
  searchRoleById(id: string) {
    ...
    this.roleService.searchById(id).subscribe({
      ...
      error: (e: any) => {
        Swal.fire('Roles', e.error.message ? `${e.error.message}` : `No se encontro ningun registro con el id ${id}`, 'error').then((result) => {
          if (result.isConfirmed) {
            console.log('Error');
          }
        });
